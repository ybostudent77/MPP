class PatientsController < ApplicationController
  ...
  def index
    patients_query = PatientsQuery.new
    @patients = patients_query.search(params[:query]).sort(params[:sort_field], params[:sort_direction]).result.paginate(page: params[:page], per_page: 10)
  end
