class SpecializationsController < ApplicationController
  ...
  def index
    specializations_query = SpecializationsQuery.new
    @specializations = specializations_query.search(params[:query]).sort(params[:sort_field], params[:sort_direction]).result.paginate(page: params[:page], per_page: 10)
  end
